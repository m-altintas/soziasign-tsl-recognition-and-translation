...
Typical usage::
    ...
    python -m src train                        # full training, default GRU
    python -m src train --test                 # quick 10-class smoke test
"""
...
def _build_scheduler(cfg: TrainConfig, optimizer: torch.optim.Optimizer, train_loader):
    ...
    if name in {"onecycle", "onecyclelr"}:
        ...
        # Warmup fraction (pct_start) derived from warmup_epochs; clamped for safety.
        warmup_epochs = max(int(cfg.warmup_epochs), 0)
        pct_start = warmup_epochs / max(int(cfg.epochs), 1)
        pct_start = float(min(max(pct_start, 1e-3), 0.5))  # keep stable; avoid degenerate schedules
        # Choose a sensible max_lr if not provided.
        # Rule of thumb for Adam: max_lr ~ 5–10x base LR. We default to 10x, capped at 1e-2.
...
def _save_scores(
    ...
    for i, label in enumerate(actions):
        ...
        support = int(tp + fn)  # total true positives for this class
        per_class[label] = {
    ...
    # Macro averages (only over classes that have test samples)
...
def _save_plots(run_dir: Path, log_rows: list[dict]) -> None:
    ...
    # Top-5 accuracy data (may be absent in legacy logs)
...
# ---------------------------------------------------------------------------
# Default hyperparameters tuned for xlarge model sizes.  These are applied
# *only* when the user hasn't explicitly overridden the corresponding field.
_XLARGE_DEFAULTS: dict[str, object] = {
    "dropout": 0.2,           # less regularization so the extra capacity is used
    "epochs": 500,            # more training time for the OneCycleLR to anneal
}
...
def _apply_xlarge_defaults(cfg: TrainConfig) -> list[str]:
    ...
    messages describing what was changed (for printing to the training log)."""
    # We only touch fields the user hasn't explicitly set (i.e. still at
...
def train(cfg: TrainConfig | None = None) -> dict:
    ...
    # ---- Seed reset (ensures determinism even across multiple runs) ----
    set_seed(SEED)
    ...
    # Resolve dropout: None → size-specific default
    if cfg.dropout is None:
        cfg.dropout = 0.4  # default for small / large
    ...
    # Save config now that we know feature_dim and param count
    ...
    best_val_acc = 0.0
    best_val_acc5 = 0.0
    best_val_epoch = 0  # epoch at which best val acc was recorded
    log_rows: list[dict] = []
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        # Validate periodically against the VALIDATION set (not test)
        ...
        if epoch == 1 or epoch % cfg.val_every == 0 or epoch == cfg.epochs:
            ...
            # ReduceLROnPlateau is stepped on validation loss
        ...
        # Epoch-stepped schedulers (cosine warm restarts / warmup)
        ...
        # Early stopping: stop if val accuracy hasn't improved for patience epochs
        # The min_epochs guard prevents premature stopping before the LR
        # scheduler (e.g. OneCycleLR) has had time to reach its low-LR regime.
        if (
    ...
    # Save end-of-training weights BEFORE restoring best checkpoint
    final_model_path = run_dir / "final_model.pt"
    torch.save(model.state_dict(), final_model_path)
    print(f"Final model saved to: {final_model_path}")

    # Then restore best weights for final evaluation
    ...
    # Quick sanity check on first test sample
    model.eval()
    with torch.no_grad():
    ...
    # ---- Save scores, training log, plots ----
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Train the SLR model")
    ...
    config = TrainConfig.test() if args.test else TrainConfig.full()
    if args.model_size:
        config.model_size_override = args.model_size
    train(config)
