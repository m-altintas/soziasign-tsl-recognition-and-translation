...
Every dataset supported by the SLR pipeline implements ``DatasetInfo``.
The base class defines the interface that the rest of the pipeline
(extraction, splitting, training, evaluation) depends on, so adding a
new dataset is just a matter of creating a new subclass and registering
it in ``__init__.py``.
"""
...
class DatasetInfo(ABC):
    ...
    def processed_dir(self) -> Path:
        ...
        The convention is ``processed/{class_name}/{sample}.npy`` so that
    ...
    def predefined_split_entries(self) -> dict[str, list[dict]] | None:
        ...
        (i.e. splits should be computed by ``split.py``).
        """
        return None
    ...
    def output_npy_path(self, class_name: str, sample_id: str) -> Path:
        ...
        Default layout: ``processed_dir / class_name / sample_id.npy``.
